...
-- Input Blocking (Blur & Sink)
local Blur = Instance.new("BlurEffect", game.Lighting)
Blur.Size = 15; Blur.Enabled = true; Library.LookBlur = Blur
...
function Library:CreateWindow(options)
    ...
    -- Render Loop
    AddConnection(RunService.RenderStepped, function() self:Update() end)
    ...
    AddConnection(UserInputService.InputChanged, function(i) 
        if i.UserInputType == Enum.UserInputType.MouseMovement then 
            ...
            elseif self.SliderDragging then
                local s = self.SliderDragging
                local pct = math.clamp((m.X - s.Click.X) / s.Click.W, 0, 1)
                local newVal = s.Min + (pct * (s.Max - s.Min))
                
                -- Rounding logic based on decimals
                -- For now just standard rounding
                newVal = math.floor(newVal * 10) / 10 
                
                -- Update Value
                s.Callback(newVal)
                Library.Flags[s.Flag] = newVal
                -- Visual update handled in render loop via reading Flag/Callback value? 
                -- We'll store value in object for render
                s.Value = newVal
            end
        end
    end)
...
function Tab:UpdateContent(wx, wy, ww, wh)
    -- Column Logic
    local colW = (ww - 40) / 3 -- Assuming 3 cols for now or 2? Original was 3.
    -- If using Left/Right, we split.
    -- Let's stick to 2 major columns or 3 like original? Original had C1, C2, C3.
    -- We can adapt 'Auto' to fill them sequentially.
    
    -- Layout: C1, C2, C3
    local curX, curY = wx + 15, wy + 60
    local columns = {self.Groups.Left, self.Groups.Right} -- Simplified to 2 cols for now or dynamic?
    -- Actually let's just use linear for Auto.
    
    -- Render all groups in 'Auto' list for simplicity in this port
    -- Or implement proper Left/Right
    local groupsToRender = self.Groups.Auto
    
    local colIdx = 0
    for _, group in ipairs(groupsToRender) do
        local gX = curX + (colIdx * (colW + 10))
        local gY = curY
        
        group:Update(gX, gY, colW)
        
        colIdx = colIdx + 1
        if colIdx >= 3 then colIdx = 0; curY = curY + 200 end -- Wrap roughly
    end
end

function Tab:HandleInput(m)
    for _, g in ipairs(self.Groups.Auto) do
        g:HandleInput(m)
    end
end
...
function Tab:CreateGroup(name)
    ...
    table.insert(self.Groups.Auto, group)
    return group
end
...
function Element.new(type, group, args)
    ...
    self.Draws = {
        Text = NewDrawing("Text", {Size=15, Font=2, Outline=true, ZIndex=4, Text=self.Name}),
        Box = NewDrawing("Square", {Filled=true, ZIndex=4}), -- For Toggle/Slider bg
        Fill = NewDrawing("Square", {Filled=true, ZIndex=5}), -- For active state
        Val = NewDrawing("Text", {Size=13, Font=2, Outline=true, ZIndex=4}) -- Value text
    }
...
function Group:AddSlider(args)
    ...
    el.Min = args.Min or 0
    el.Max = args.Max or 100
    el.Value = args.Default or el.Min
    Library.Flags[el.Flag] = el.Value
    ...
    function el:Update(x, y, w)
        ...
        self.Draws.Val.Text = tostring(math.floor(self.Value*10)/10)
        self.Draws.Val.Position = Vector2.new(x+w-30, y)
        self.Draws.Val.Visible = true
    end
    ...
    function el:HandleInput(m)
        local c = self.ClickArea
        if m.X >= c.X and m.X <= c.X+c.W and m.Y >= c.Y and m.Y <= c.Y+c.H then
            self.Group.Parent.Parent.SliderDragging = self -- Link to Window
            return true
        end
    end
